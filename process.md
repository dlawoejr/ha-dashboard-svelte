...
## 🎨 스케줄러 개선, 슬라이더 로직 보정 및 디자인 고도화 (Phase 1~8)

> 작업일: 2026-03-02

### 🎯 목표

스케줄러 UI를 더 직관적이고 편리하게 개선하고, 하드웨어 보호를 위한 슬라이더 제어 로직을 구현하며, 전체적인 디자인을 '스마트팜' 브랜드에 맞춰 고도화한다.

### 📋 주요 구현 및 개선 내용

#### 1. 스케줄러(Scheduler) UI/UX 고도화
- **하이브리드 타임 피커 ([이미지 기반](file:///C:/Users/sysk/.gemini/antigravity/brain/b88eb765-f542-46ca-8fd1-059da140de5c/media__1772425472619.png))**: 위/아래 화살표(스피너)와 직접 입력창을 결합하여 조작 편의성 증대.
- **5분 단위 스냅(Snapping) 로직**: 화살표 조절 시 분 단위를 가장 가까운 5분 배수로 자동 정렬 (예: 43분 → 위 클릭 시 45분, 아래 클릭 시 40분).
- **편집 모드 최적화**: 수정 화면에서 불필요한 기기 검색/선택 기능을 제거하고 현재 기기 명칭을 고정 노출하여 혼동 방지.
- **커스텀 삭제 확인 모달**: 브라우저 기본 `confirm()`을 대시보드 테마에 맞는 세련된 모달로 교체.
- **모달 백드롭 보호**: 모달 밖 클릭 시 창이 예기치 않게 닫히는 것을 방지하여 데이터 유실 방지.

#### 2. 슬라이더(Slider) 제어 로직 보정
- **하드웨어 보호 로직**: '열기'에서 '닫기'로(또는 반대) 급격하게 슬라이딩할 경우, 자동으로 **'정지(Stop)'** 위치에서 멈추도록 강제하여 모터/하드웨어 무리 방지.

#### 3. 브랜딩 및 디자인 시스템 통일
- **브랜드 변경**: 앱 명칭을 `HA Dashboard`에서 정식 브랜드인 **`스마트팜`**으로 변경 및 테스트 라벨 제거.
- **타이틀 디자인**: 헤더 타이틀의 폰트 크기(1.1rem)와 두께(600)를 사이드바 항목과 1:1로 일치시켜 시각적 안정성 확보.
- **영역(Area) 탭 동기화**: 대시보드 상단 탭의 활성화 스타일을 사이드바의 보라색 그라데이션 및 그림자 효과와 통일.
- **UI 클린업**: 선택되지 않은 탭의 배경색(Hover 포함)을 제거하여 강조 항목의 시인성 극대화.
- **영역 테두리 유지**: 비활성 탭에 은은한 테두리를 적용하여 깔끔하면서도 구분감이 있는 디자인 구현.

#### 4. 모바일 레이아웃 최적화
- **헤더 정렬**: 모바일 화면에서 타이틀 줄바꿈(개행)을 방지하고, 연결 상태 배지(`connected`)를 우측 끝으로 배치하여 공간 효율화.
- **독립 스크롤**: 전체 페이지 스크롤을 막고 본문 컨텐츠 영역만 스크롤되도록 CSS 레이아웃 개편 (모바일 UX 개선).

### 📁 변경된 주요 파일

| 파일 | 역할 |
|---|---|
| `src/lib/components/SchedulerCard.svelte` | 하이브리드 타임 피커, 편집/삭제 로직, 레이아웃 수정 |
